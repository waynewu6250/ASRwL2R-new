...
from model.ptranking.base.ranker import LTRFRAME_TYPE
from model.ptranking.utils.bigdata.BigPickle import pickle_save
from model.ptranking.metric.metric_utils import metric_results_to_string
from model.ptranking.data.data_utils import get_data_meta, SPLIT_TYPE, LABEL_TYPE
from model.ptranking.ltr_adhoc.eval.eval_utils import ndcg_at_ks, ndcg_at_k, calc_wer
from model.ptranking.data.data_utils import LTRDataset, YAHOO_LTR, ISTELLA_LTR, MSLETOR_SEMI, MSLETOR_LIST
from model.ptranking.ltr_adhoc.eval.parameter import ModelParameter, DataSetting, EvalSetting, ScoringFunctionParameter

from model.ptranking.ltr_adhoc.listwise.rank_cosine import RankCosine
from model.ptranking.ltr_adhoc.listwise.listnet     import ListNet
from model.ptranking.ltr_adhoc.listwise.listmle     import ListMLE, ListMLEParameter
from model.ptranking.ltr_adhoc.listwise.st_listnet  import STListNet, STListNetParameter

from model.ptranking.ltr_adhoc.listwise.approxNDCG        import ApproxNDCG, ApproxNDCGParameter
from model.ptranking.ltr_adhoc.listwise.wassrank.wassRank import WassRank, WassRankParameter
from model.ptranking.ltr_adhoc.listwise.lambdarank        import LambdaRank, LambdaRankParameter
from model.ptranking.ltr_adhoc.listwise.lambdaloss import LambdaLoss, LambdaLossParameter
...
class LTREvaluator():
    ...
    def determine_files(self, data_dict, fold_k=None):
        ...
        if data_dict['data_id'] in YAHOO_LTR:
            file_train, file_vali, file_test = os.path.join(data_dict['dir_data'], 'train.txt'),\
                                               os.path.join(data_dict['dir_data'], 'valid.txt'),\
                                               os.path.join(data_dict['dir_data'], 'test.txt')
        ...
        elif data_dict['data_id'].find('MQ2008') != -1:
            print('Fold-', fold_k)
    ...
    def kfold_cv_eval(self, data_dict=None, eval_dict=None, sf_para_dict=None, model_para_dict=None, dev_data=None):
        """
        ...
        for fold_k in range(1, fold_num + 1):   # evaluation over k-fold data
            ...
            print('training utterances: ', len(train_data))
            print('validating utterances: ', len(vali_data))
            print('testing utterances: ', len(test_data))
            ...
            # fold_optimal_checkpoint = 'Fold-1'
            # fold_optimal_epoch_val = 62
            ...
            # wer
            calc_wer(ranker=fold_optimal_ranker, test_data=test_data, gpu=self.gpu, device=self.device, test_data_df=dev_data)
    ...
    def grid_run(self, model_id=None, dir_json=None, debug=False, data_id=None, dir_data=None, dir_output=None, dev_data=None):
        """
                ...
                for sf_para_dict in self.iterate_scoring_function_setting(data_dict=data_dict):
                    for model_para_dict in self.iterate_model_setting():
                        curr_cv_avg_scores = self.kfold_cv_eval(data_dict=data_dict, eval_dict=eval_dict,
                                                            sf_para_dict=sf_para_dict, model_para_dict=model_para_dict, dev_data=dev_data)
    ...
    def run(self, debug=False, model_id=None, config_with_json=None, dir_json=None,
            data_id=None, dir_data=None, dir_output=None, grid_search=False, dev_data=None):
        if config_with_json:
            assert dir_json is not None
            self.grid_run(debug=debug, model_id=model_id, dir_json=dir_json, dev_data=dev_data)
        else:
            if grid_search:
