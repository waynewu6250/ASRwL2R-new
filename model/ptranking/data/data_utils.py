...
from model.ptranking.ltr_adhoc.util.bin_utils import batch_count
from model.ptranking.utils.numpy.np_extensions import np_arg_shuffle_ties
from model.ptranking.ltr_adhoc.util.one_hot_utils import get_one_hot_reprs
from model.ptranking.utils.bigdata.BigPickle import pickle_save, pickle_load
...
MSLETOR       = ['MQ2007_Super', 'MQ2008_Super', 'MQ2007_Semi', 'MQ2008_Semi', 'MQ2007_List', 'MQ2008_List', 'Alexa14']
...
def get_data_meta(data_id=None):
    ...
    if data_id in MSLRWEB:
        ...
        max_rele_level = 5
        label_type = LABEL_TYPE.MultiLabel
        num_features = 14 # libsvm format, rather than uniform number
        has_comment = False
        fold_num = 1
    ...
    elif data_id in ISTELLA_LTR or data_id in GLTR_LETOR:
        max_rele_level = 5
        label_type = LABEL_TYPE.MultiLabel
        num_features = 14  # libsvm format, rather than uniform number
        fold_num = 1
    ...
    with open(in_file, encoding='iso-8859-1') as file_obj:
        ...
        if has_comment:
            ...
            if data_dict['data_id'] in YAHOO_LTR:
                all_features_mat, all_labels_vec, qids = parse_letor(file_obj.readlines(), has_comment=False, one_indexed=True)
            else:
...
class LTRDataset(data.Dataset):
    ...
    def __init__(self, split_type, file, data_id=None, data_dict=None, eval_dict=None, batch_size=1, presort=False,
        ...
        if data_dict['data_id'] in MSLETOR or data_dict['data_id'] in MSLRWEB \
                ...
                or data_dict['data_id'] == 'IRGAN_MQ2008_Semi' \
                or data_dict['data_id'] in GLTR_LETOR: # supported datasets
            ...
            a = True
            if not a: #os.path.exists(torch_perquery_file):
