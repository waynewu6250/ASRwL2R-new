...
class CoreDataset(Dataset):
    ...
    def __init__(self, dataset, utterance_mode='group', embed='bert', audio_dic=None):
        ...
        :parameter:
            ...
            audio_embeddings: a list of audio embeddings
        """
        self.maxlen = 20
        ...
        for column in ["neg_confidence", "neg_confidence_stable", "neg_directedness", "neg_directedness_stable"]:
            self.sparse_features.loc[:][column] = self.sparse_features.loc[:][column].replace(np.NaN, self.sparse_features[column].mean())

        # Set a new column by replacing truth to rewrite if truth is None
        self.data['truth_update'] = np.where(self.data['truth'] != '', self.data['truth'], self.data['rewrite'])
        ...
        if self.utterance_mode == 'single':
            ...
            for i, (qid, a, b) in enumerate(query_groups):
                ...
                ranks = self.data.loc[a:b-1]['rank'].values
                tokenized_data = self.tokenize_and_pad_data(hypotheses)
                self.utterances.append(tokenized_data)

                # ground truth
                tokenized_truth = self.tokenize_and_pad_data([self.data.loc[a]['truth_update']])
    ...
    def __getitem__(self, index):
        ...
        if self.utterance_mode == 'single':
            ...
            return self.utterances[index], self.audio_embeddings[index], self.ranks[index], self.truths[index], self.asr_features[index]
...
def collate_fn(batch):
    ...
    utterances, audio_embeddings, ranks, truths, asr_features = zip(*batch)
    ...
    result_ranks = t.zeros((len(utterances), max_len))
    ...
    return result_ids, result_token_masks, result_masks, lengths, result_audio, result_ranks, truth_ids, truth_token_masks, result_asr_features
...
if __name__ == '__main__':
    ...
    for i, (result_ids, result_token_masks, result_masks,
            lengths, result_audio, result_ranks,
